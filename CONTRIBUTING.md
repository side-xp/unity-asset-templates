...
3. Open the project in Unity. This package is **editor-only**: all of its code lives in the `Editor/` folder and is compiled into a single editor assembly (suffixed `.Editor`) through its `*.asmdef` file.
...
# This package is editor-only, so only the Editor/ folder is scanned. Keep the --input
# folders in sync with the package's actual source folders (and with the release workflow).
instadoc \
  --input Editor \
